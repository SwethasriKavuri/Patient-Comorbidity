...
//var helpimg = "\img/help.png";
//var closeimg = "\img/close.png";
...
var point_of_admission_order = null;
...
/**
 * Populates the values of all the variables from the JSON object.
 * @param {Object} jsonObj - JSON object containing all the values of the variables.
 */
function getData(jsonObj){
	
		console.log(jsonObj);
		if (jsonObj){
			point_of_admission_order = jsonObj.TEMP.POINT_OF_ADMISSION_ORDER;
			...
			maxplt = jsonObj.TEMP.RESULT_MAX_VAL_PLT;
			minplt = jsonObj.TEMP.RESULT_MIN_VAL_PLT;
			maxpltDate = jsonObj.TEMP.EVENT_PLT_MAX_DATE;
			minpltDate = jsonObj.TEMP.EVENT_PLT_MIN_DATE;
			pltLow = jsonObj.TEMP.NORMAL_LOW_PLT;
			pltHigh = jsonObj.TEMP.NORMAL_HIGH_PLT;
				
			maxptinr = jsonObj.TEMP.RESULT_MAX_VAL_PTINR;
			minptinr = jsonObj.TEMP.RESULT_MIN_VAL_PTINR;
			maxptinrDate = jsonObj.TEMP.EVENT_PTINR_MAX_DATE;
			minptinrDate = jsonObj.TEMP.EVENT_PTINR_MIN_DATE;
			ptinrLow = jsonObj.TEMP.NORMAL_LOW_PTINR;
			ptinrHigh = jsonObj.TEMP.NORMAL_HIGH_PTINR;
...
/**
 * Builds the header of the Previously Committed Comorbidities section.
 * Header consists of the text "Previously Committed CoMorbidities/Clinical Conditions" and a down-arrow, which collapses the body of the Previously Committed Comorbidities section on click.
 */
...
/**
 * Builds the body of the Previously Committed Comorbidities section.
 * @param {Object} jsonObj - JSON object representing the values of various events/ observations/ variables.
 */
...
/**
 * Builds the header of the ED Comorbidities section.
 * Header consists of the text "ED CoMorbidities/Clinical Conditions" and a down-arrow, which collapses the body of the ED Comorbidities section on click.
 */
function buildEDheader(){
var tableBody = "";
tableBody += "<table width='100%' bgcolor='white'>"
		  + "<tr><td><br><p class='test' onclick='javascript:expandcollapse(\"edarrow\",\"edbody\");'>ED CoMorbidities&nbsp;&nbsp;"
		  + "<img id='edarrow' border='0' src='"+arrowdownimg+"' alt='Click to collapse' /></p></td></tr>"
		  + "</table>";
$('#edheader').html(tableBody);
return;
}

function buildEDbody(jsonObj){

	$('#edbody').html("");
	var tableBody = "";

	//console.log(Date.now());
	//console.log("Point of admission : " + point_of_admission_order);

	var current_time_milliseconds = Date.now();
	//console.log("Millis : " + current_time_milliseconds);

	var point_of_admission_order_millis = new Date(point_of_admission_order.replace("_", " "));
	//console.log("Point of admission after formatting : " + point_of_admission_order_millis);

	//timeDifferenceCheck(current_time_milliseconds, point_of_admission_order_millis.getTime());

	if(timeDifferenceCheck(current_time_milliseconds, point_of_admission_order_millis.getTime()))
		console.log("Diff < 24 hrs");
	else
		console.log("Diff > 24 hrs"); 
	...
	if(timeDifferenceCheck(current_time_milliseconds, point_of_admission_order_millis.getTime())) {
		...
		if (jsonObj.TEMP.REC_ED_CNT > 0){
			tableBody += "<table width='100%' bgcolor='white'><col width='10%'><col width='90%'><tr><td></td><td>"
					  + "<table width='100%' border='0' style='font-family:sans-serif;font-size:16px'>";
			for (var i = 0; i < jsonObj.TEMP.REC_ED_CNT; i++){
				for (var k = 0; k < jsonObj.TEMP.REC_ED[i].COMORB_CNT; k++){
					if (jsonObj.TEMP.REC_ED[i].COMORB[k].RES_ED_CNT > 0){
						tableBody += "<tr><td><table><tr onmouseover=\"mouseOverPara('"+i+"edimageAbl"+k+"','"+i+"edablCol"+k+"');\" onmouseout=\"mouseOutPara('"+i+"edimageAbl"+k+"','"+i+"edablCol"+k+"')\">"
								  + "<td id='"+i+"edablCol"+k+"'><span style='color:blue;font-size:13px'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;"
								  + "<a onmouseover=\"makeDivVisible('"+i+"edabl"+k+"');\" onmouseout=\"makeDivHidden('"+i+"edabl"+k+"')\">"+jsonObj.TEMP.REC_ED[i].COMORB[k].DESC+"</a></span></td>"
								  + "<td><a id='"+i+"edimageAbl"+k+"' onmouseover=\"makeDivVisible('"+i+"edabl"+k+"');\" onmouseout=\"makeDivHidden('"+i+"edabl"+k+"');\" style='visibility: hidden;'> "
								  + "<img src='"+button1img+"' alt='Details'/></a></td>"
								  + "<td><div id='"+i+"edabl"+k+"' class='imageBack'><img src='"+divimg+"' width='100%' height='100%'/>"
								  + "<div id='"+i+"edabl"+k+"' class='dynamicDiv' style='width:280px; height:230px; margin-left: 45px ; margin-top: -275px;'>"
								  + "<br><span style='color:#858384; font-weight:bold;'>"
								  + "Values are:<br><br></span>";
						for (var m = 0; m < jsonObj.TEMP.REC_ED[i].COMORB[k].RES_ED_CNT; m++){
							tableBody += jsonObj.TEMP.REC_ED[i].COMORB[k].RES_ED[m].RESULT + "<br>";
							}
						tableBody += "</div></div></td></tr></table></td></tr>";
						}
					else{
						tableBody += "<tr><td style='font-weight:bold;font-size:13px'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + jsonObj.TEMP.REC_ED[i].COMORB[k].DESC + "</td></tr>";
						}
					}
				tableBody += "<tr></tr>";
				}
			tableBody += "</table></td></tr></table>";
			$('#edbody').html(tableBody);
		}
	}	
...
/**
 * Builds the header of the Computed Comorbidities section.
 * Header consists of the text "Computed CoMorbidities/Clinical Conditions" and a down-arrow, which collapses the body of the Computed Comorbidities section on click.
 */
...
/**
 * Builds the body of the Computed Comorbidities section, by checking the rules for Anemia, Kidney Injury, Malnutrition, etc.
 */
...
//<!-- ********************************************* Thrombocytopenia ************************************************* -->
if ($.inArray("^Thrombocytopenia^",pcArr) == -1){
	result = Thrombocytopenia(maxplt);
	/*
	result=1
	maxplt=1
	*/
	if (result == 1){
		...
		tableBody += "<tr><td><table>" 
				  + "<tr onmouseover=\"mouseOverPara('imageThrombocytes','ThrombocyteCol1');\" onmouseout=\"mouseOutPara('imageThrombocytes','ThrombocyteCol1');\">"
				  + "<td id='ThrombocyteCol1'>"
				  + "<span style='color: blue;'>"
				  + "<a onmouseover=\"makeDivVisible('Thrombocyte');\" onmouseout=\"makeDivHidden('Thrombocyte');\"><input type='checkbox' value='Thrombocytopenia'>"
				  + "Thrombocytopenia</a></span>"
				  + "</td>"
				  + "<td><a id='imageThrombocytes' onmouseover=\"makeDivVisible('Thrombocyte');\" onmouseout=\"makeDivHidden('Thrombocyte');\" style='visibility: hidden;'>"
				  + "<img src='"+button1img+"' alt='Details'/></a></td>"
				  + "<td><div id='Thrombocyte' class='imageBackSingle' style='width:340px; height:270px;'><img src='"+divSingleimg+"' width='100%' height='100%'/>"
				  + "<div id='Thrombocyte' class='dynamicDiv' style='width:250px; height:210px; margin-left: 40px ; margin-top: -247px;'>"
				  + "<br><span style='color: #858384; font-weight: bold;'>"
				  + "Values are:<br><br></span>"
				  + "Total Platelet Count: "+maxplt+" "+ maxpltDate +"<br>"
				  + "<input type='hidden' name='maxpltDate' id='maxpltDate' value='"+maxpltDate+"'>"
				  + "<input type='hidden' name='pltResult' id='pltResult' value='"+maxplt+"'>"
				  + "<br><span style='color: #858384; font-weight: bold;'> "
				  + "Reference Range:<br><br></span>"
				  + "Total Platelet Count < 150"
				  + "</div>"
				  + "</div>"
				  + "</td></tr></table></td></tr>";
		} 
...
//<!-- ********************************************* Coagulopathy ************************************************* -->
if ($.inArray("^Coagulopathy^",pcArr) == -1){
	result = Coagulopathy(minptinr);
	
	if (result == 1){
		newCC = 1;
		tableBody += "<tr><td><table>" 
				  + "<tr onmouseover=\"mouseOverPara('imageCoagulopathy','CoagulopathyCol1');\" onmouseout=\"mouseOutPara('imageCoagulopathy','CoagulopathyCol1');\">"
				  + "<td id='CoagulopathyCol1'>"
				  + "<span style='color: blue;'>"
				  + "<a onmouseover=\"makeDivVisible('Coagulopathy');\" onmouseout=\"makeDivHidden('Coagulopathy');\"><input type='checkbox' value='Coagulopathy'>"
				  + "Coagulopathy</a></span>"
				  + "</td>"
				  + "<td><a id='imageCoagulopathy' onmouseover=\"makeDivVisible('Coagulopathy');\" onmouseout=\"makeDivHidden('Coagulopathy');\" style='visibility: hidden;'>"
				  + "<img src='"+button1img+"' alt='Details'/></a></td>"
				  + "<td><div id='Coagulopathy' class='imageBackSingle' style='width:340px; height:270px;'><img src='"+divSingleimg+"' width='100%' height='100%'/>"
				  + "<div id='Coagulopathy' class='dynamicDiv' style='width:250px; height:210px; margin-left: 40px ; margin-top: -247px;'>"
				  + "<br><span style='color: #858384; font-weight: bold;'>"
				  + "Values are:<br><br></span>"
				  + "MIN INR: "+minptinr+" "+ minptinrDate +"<br>"
				  + "<input type='hidden' name='minptinrDate' id='minptinrDate' value='"+minptinrDate+"'>"
				  + "<input type='hidden' name='inrResult' id='inrResult' value='"+minptinr+"'>"
				  + "<br><span style='color: #858384; font-weight: bold;'> "
				  + "Reference Range:<br><br></span>"
				  + "MIN INR RATE >= 1.3"
				  + "</div>"
...
if ($.inArray("^Kidney Injury^",pcArr) == -1){
	...
	maxcreatResult = "1.75";
	maxcreatprevResult = "1.11";

	result = kidneyInjury(maxcreatResult,maxcreatprevResult, creatHigh);

	console.log(parseDouble(maxcreatResult));
	console.log(parseDouble(maxcreatprevResult));
	console.log(parseDouble(creatHigh));
	console.log(result);
	...
	if (result != 0){
		...
		tableBody += "<tr><td><table><tr><td>"
				  + "<span style='color: black'>"
				  + "<input type='checkbox' id='ckd' value='GROUP' onclick=\"unCheck('ckd','ckdChkGrp');\">Kidney Disease :</span>"
		...
		if (result == 2){
			tableBody += "<tr onmouseover=\"mouseOverPara('imageRfAcute','RfAcuteCol');\" onmouseout=\"mouseOutPara('imageRfAcute','RfAcuteCol');\">"
					  ...
					  + "<input type='checkbox' value='Kidney Disease:Acute' onclick=\"checkMain('ckd','ckdChkGrp');\">"
					  + "Acute&nbsp;&nbsp;</a></span>";
		}
		else{
			tableBody += "<tr onmouseover=\"mouseOverPara('imageRfAcute','RfAcuteCol');\" onmouseout=\"mouseOutPara('imageRfAcute','RfAcuteCol');\">"
		...
		tableBody += "</td>"
					  ...
					  + "<img src='"+button1img+"' alt='Details'/></a></td>"
					  + "<td><div id='RfAcute' class='imageBack' style='width:430px; height:330px;'><img src='"+divimg+"' width='100%' height='100%'/>"
					  + "<div id='RfAcute' class='dynamicDiv' style='width:350px; height:270px; margin-top: -305px;'>"
					  + "<br><span style='color: #858384; font-weight: bold;'>"
					  + "Values are:<br><br></span> "
			...
			tableBody += "<br><span style='color: #858384; font-weight: bold;'> "
					  + "Determined by the Rule:<br><br></span>"
					  + "Current creatinine > " + creatHigh + "and"
					  + "Previous creatinine <=" + creatHigh + "<br>"
					  + "</div>"
					  + "</div>"
					  + "</td></tr>";
		...
		if (result == 3){
			...
			tableBody += "<tr onmouseover=\"mouseOverPara('imageRfChronic','RfChronicCol');\" onmouseout=\"mouseOutPara('imageRfChronic','RfChronicCol');\">"
				  + "<td id='RfChronicCol'><span style='color: black;'>"
				  + "<a onmouseover=\"makeDivVisible('RfChronic');\" onmouseout=\"makeDivHidden('RfChronic');\">"
				  + "<input type='checkbox' value='Chronic Kidney Disease:Chronic' onclick=\"checkMain('ckd','ckdChkGrp');\">"
				  + "Chronic&nbsp;&nbsp;</a></span>";
		}
		...
		if (result == 1){
			...
			tableBody += "<tr onmouseover=\"mouseOverPara('imageRfChronicNOS','RfChronicNOSCol');\" onmouseout=\"mouseOutPara('imageRfChronicNOS','RfChronicNOSCol');\">"
				  + "<td id='RfChronicNOSCol'><span style='color: black;'>"
				  + "<a onmouseover=\"makeDivVisible('RfChronicNOS');\" onmouseout=\"makeDivHidden('RfChronicNOS');\">"
				  + "<input type='checkbox' value='Chronic Kidney Disease:NOS' onclick=\"checkMain('ckd','ckdChkGrp');\">"
				  + "Not Otherwise Specified&nbsp;&nbsp;</a></span>";
		}
...
function timeDifferenceCheck(date1, date2) {
        ...
        var secondsDifference = Math.floor(difference/1000);
     	...
     	if(daysDifference < 1) {
     		...
     		if (daysDifference == 1 && hoursDifference == 0 && minutesDifference == 0 && secondsDifference == 0) {
     			return true;
     		}
     		else {
     			return false;
...
/**
 * Builds the header of the Other Comorbidities section.
 * Header consists of the text "Other CoMorbidities/Clinical Conditions" and a down-arrow, which collapses the body of the Other Comorbidities section on click.
 */
...
/**
 * Builds the body of the Other Comorbidities section.
 */
...
/**
 * Expands and collapses the body of a section, when the down-arrow is clicked.
 * @param {string} header - Name of the header section
 * @param {string} body - Name of the body section
 */
...
/**
 * Unchecks the main checkbox and the subgroup checkbox. 
 * @param {string} checkBoxMain - Input tag ID of the main checkbox.
 * @param {@string} subGrp - Input tag ID of the subgroup checkbox.
 */
...
/**
 * Checks the main checkbox and the subgroup checkbox.
 * @param {string} checkBoxMain - Input tag ID of the main checkbox.
 * @param {@string} subGrp - Input tag ID of the subgroup checkbox.
 */
...
/**
 * Displays the sub co-morbidities checkboxes, when a co-morbidity checkbox is checked.
 * @param {string} comorbId - Input tag ID of the sub co-morbidities checkbox group.
 * @param {string} input - Input tag ID of the co-morbidity checkbox.
 */
...
/**
 * Displays the image, when the mouse is moved over a row of Other Comorbidities body section.
 * @param {string} imageID - Input ID tag of the image.
 * @param {string} col1 - Input ID tag of the table row.
 */
...
/**
 * Hides the image, when the mouse is moved out of a row of Other Comorbidities body section.
 * @param {string} imageID - Input ID tag of the image.
 * @param {string} col1 - Input ID tag of the table row.
 */
...
/**
 * Makes the HTML Div element visible.
 * @param {string} divId - Input tag ID of the HTML Div element
 */
...
/**
 * Makes the HTML Div element invisible.
 * @param {string} divId - Input tag ID of the HTML Div element
 */
...
/**
 * Displays the help guide on clicking the Help icon at the top of the webpage.
 * @param {string} comorb - String representing the comorbidity detected for a patient.
 */
...
if (comorb == "Anemia"){
	tableBody += "<h2 align='center'>Acute Blood Loss Anemia</h2><br>"
			  ...
			  + "<ul><li><p>Acute: if min hb < normal low or min hc < normal low and both previous min hb and previous min hc are available and both previous min hb > normal low and previous min hc > normal low</p></li>"
              + "<li><p>Chronic: if min hb < normal low or min hc < normal low and both previous min hb and previous min hc are available and (previous min hb < normal low or previous min hc < normal low)</p></li>"
			  + "<li>if min hb < normal low or min hc < normal low and both previous min hb > normal low and previous min hc > normal low</li></ul>";
	}
...
else if (comorb == "Thrombocytopenia"){
	tableBody += "<h2 align='center'>Thrombocytopenia</h2><br>"
			  + "<p align='center'>Following are the conditions for Thrombocytopenia:</p>"
			  + "<ul><li>If Platelet Count < 150</li></ul>";
	}

else if (comorb == "Coagulopathy"){
	tableBody += "<h2 align='center'>Coagulopathy</h2><br>"
			  + "<p align='center'>Following are the conditions for Coagulopathy:</p>"
			  + "<ul><li>If INR >= 1.3 </li></ul>";
	}
...
/**
 * Displays the quick help guide inside the Help guide.
 * @param {string} comorb - String representing the comorbidity detected for a patient.
 */
...
/**
 * Commits the selected co-morbidities from the Computed co-morbidities and Other co-morbidities to the database.
 */
function commit(){
var d = new Date();
...
/**
 * Submits the computed and other co-morbidities to the database.
 * @param {string} val - String representing the computed co-morbidities.
 * @param {string} ocval - String representing the other co-morbidities.
 * @param {Object} currentdate - Object representing the date-time when the co-morbidities was submitted to the database.
 */
function addCE(val,ocval,radval,currentdate){
...
if (val.indexOf("^Thrombocytopenia^") > -1){
	comorbidities += "~Thrombocytopenia^";
	comorbidities += ",Total Platelet Count: " + maxplt + " " + maxpltDate;
	}

if (val.indexOf("^Coagulopathy^") > -1){
	comorbidities += "~Coagulopathy^";
	comorbidities += ",INR: " + minptinr + " " + minptinrDate;
	}
