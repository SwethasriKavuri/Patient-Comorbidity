...
if (isEmpty(maxcreatResult) != true && isDouble(maxcreatResult) && isDouble(creatHigh)){
		...
		if (parseDouble(maxcreatResult) > parseDouble(creatHigh) && isEmpty(maxcreatprevResult) != true && parseDouble(maxcreatprevResult) > parseDouble(creatHigh)
			&& parseDouble(maxcreatResult) > (1.5 * parseDouble(maxcreatprevResult))){
			return 4; // 4 is for acute on chronic
			}
		else if ((parseDouble(maxcreatResult) > parseDouble(creatHigh) && isEmpty(maxcreatprevResult) != true && parseDouble(maxcreatprevResult) <= parseDouble(creatHigh))
				 || (parseDouble(maxcreatResult) > (1.5 * parseDouble(maxcreatprevResult)))){
			return 2; // 2 is for acute
			}
		else if (parseDouble(maxcreatResult) > parseDouble(creatHigh) && isEmpty(maxcreatprevResult)){
			return 2; // 2 is for acute
			}
		else if (parseDouble(maxcreatResult) > parseDouble(creatHigh) && isEmpty(maxcreatprevResult) != true && parseDouble(maxcreatprevResult) > parseDouble(creatHigh)){
