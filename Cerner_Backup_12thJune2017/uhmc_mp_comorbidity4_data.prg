...
drop program uhmc_mp_comorbidity4_data:dba go
create program uhmc_mp_comorbidity4_data:dba
...
head ce.event_id
	repcnt = repcnt + 1
	stat = alterlist(temp->rep,repcnt)
	if (size(trim(ce.event_title_text,3)) > 0)
		temp->rep[repcnt]->title = trim(ce.event_title_text,3)
	else
		temp->rep[repcnt]->title = trim(uar_get_code_display(ce.event_cd),3)
	endif
	temp->rep[repcnt]->link = build("javascript:showReport(",ce.person_id,",",ce.event_id,");")
detail
	...
	if (size(temp->rep[repcnt].contents) > 0)
		temp->rep[repcnt]->contents = concat(temp->rep[repcnt].contents,char(10),char(10),blob_un)
	else
		temp->rep[repcnt]->contents = blob_un
	endif
with nocounter
 
set temp->repcnt = repcnt
