...
function showRRbody(i) {
	
	var pdfContent = pdfList[i];
	
	//hidePDF();
	
	var rrbody = document.getElementById("rrbody");
	rrbody.style.display = "block";
	buildRRbody(pdfContent);
...
function buildRRlist() {
	...
	var tableBody = "";
	tableBody += "<table bgcolor='white' style='text-align:center; border-collapse:collapse; border: 1px solid #cc0000;'>";
	for (var i=0; i<pdfList.length; i++) {
		var j=i+1;
		tableBody += "<tr><td width='100px' style='border: 1px solid #cc0000;'><a id='report"+i+"' onclick='javascript:showRRbody("+i+");'>Report "+j+"</a></td></tr>";
	}
	tableBody += "</table>";
	...
	if(comorb_array != null) {
		...
		if(final_comorb != null && final_comorb.length != 0) {
			for(var i=0; i<final_comorb.length; i++) {
				...
				/*tableBody += "<tr><td><br>"
						+ "<input class='rrPart' id='rrGroup"+i+"' type='checkbox' value='GROUP' onclick='javascript:otherForm(\"rrChkGrp"+i+"\",\"rrGroup"+i+"\");'>"+final_comorb[i][0]
						+ "<div id='rrChkGrp"+i+"' style='display: none; margin-left: 60px;'>";*/
