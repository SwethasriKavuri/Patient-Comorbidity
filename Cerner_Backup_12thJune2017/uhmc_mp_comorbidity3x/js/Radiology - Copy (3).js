...
function initRR() {
	...
	showRRbody();
	var t3 = d.getTIme();
	data1 += "Time to build comorb from data" + (t3-t2).toString(); + "<br>";
	...
	tableBody += "<tr><td><b>Data :</b> data1</td></tr>";
	...
	if(comorb_array != null) {
		...
		if(final_comorb != null && final_comorb.length != 0) {
			for(var i=0; i<final_comorb.length; i++) {
				...
				tableBody += "<div id='rrChkGrp"+index+i+"' style='display: none; margin-left: 60px;'>";
				
				for(var j=1; j<final_comorb[i].length; j++) {
					tableBody += "<br><input class='rrPart"+index+i+"' id='rrGrp"+index+i+"Chk"+j+"' type='checkbox'>"+final_comorb[i][j];
				}
				tableBody += "</div></td><td style='vertical-align:top'>" 
						+ "<a id='rrSentencesImg"+index+i+"' onmouseover=\"makeDivVisible('rrSentences"+index+i+"');\" onmouseout=\"makeDivHiddenRR('rrSentences"+index+i+"');\" style='visibility: hidden;'>"
