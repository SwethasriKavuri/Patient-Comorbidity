...
var combination = final_comorb.concat(final_comorb1);		
		var array3 = arrayUnique(final_comorb.concat(final_comorb1));		
		console.log("arraay3 is:",array3);		
		...
		if(final_comorb != null && final_comorb.length != 0) {
			for(var i=0; i<final_comorb.length; i++) {
				...
				tableBody += "</div></td><td style='vertical-align:top'>" 
						...
						+ "<div id='rrSentences"+index+impression_prefix+i+"' class='dynamicDiv' style='width:370px; height:280px; margin-left: 45px ; margin-top: -275px;float: left;clear: left;'>"
	...
	if(final_comorb1 != null && final_comorb1.length != 0) {
			for(var i=0; i<final_comorb1.length; i++) {
				...
				for(var j=2; j<final_comorb1[i].length; j++) {
					...
					tableBody += "<br><input class='rrPart"+index+findings_prefix+i+"' id='rrGrp"+index+findings_prefix+i+"Chk"+j+"' type='checkbox'>"+z[i][1];
					}
				}
...
function func_category(index){
	...
	if ((category.search(chest)!=-1) || (category.search(chest.toLowerCase()) != -1) || (category.search(chest.toUpperCase()) != -1)){
		...
		two_d_array=category_processing(str_terms_comorbidities);
	}
	return two_d_array;
