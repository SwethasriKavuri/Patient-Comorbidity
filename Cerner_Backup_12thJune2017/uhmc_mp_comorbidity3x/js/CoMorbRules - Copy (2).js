...
//************************************** BP Alert *******************************************
function BPalert(maxsysBP, maxdiasBP, SYS_BP_MAX, DIAS_BP_MAX) {
	var result = 0;
	if(!(isEmpty(maxsysBP)) && !(isEmpty(maxdiasBP))) {
		if(parseDouble(maxsysBP)>parseDouble(SYS_BP_MAX) && parseDouble(maxdiasBP)>parseDouble(DIAS_BP_MAX)) {
			result = 3;
		}
		else if(parseDouble(maxsysBP)>parseDouble(SYS_BP_MAX)) {
			result = 1;
		}
		else if(parseDouble(maxdiasBP)>parseDouble(DIAS_BP_MAX)) {
			result = 2;
		}
	}
	return result;
...
//if (!isEmpty(maxcreatResult) && !isEmpty(maxcreatprevResult)){
...
if (isDouble(maxglucResult)){
	if (parseDouble(maxglucResult) > 120){ return 1; }
	}
...
// Check for Heart Rate < 40 or > 90		
if (isEmpty(minhRate) != true && isDouble(minhRate) && parseDouble(minhRate) < 40.0 ){ newminimumhrate = true; }
else if ((isEmpty(maxhRate)) != true && isDouble(maxhRate) && parseDouble(maxhRate) > 90.0){ newmaximumhrate = true; }
...
// Check for temp < 36.0 or > 38.0
if (isEmpty(maxtem) != true && isDouble(maxtem) && parseDouble(maxtem) > 38.0){ newmaximumtemp = true; }
else if (isEmpty(mintem) != true && isDouble(mintem) && parseDouble(mintem) < 36.0){ newminimumtemp = true; }
