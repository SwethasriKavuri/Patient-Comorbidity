		...
		if(final_comorb != null && final_comorb.length != 0) {
			for(var i=0; i<final_comorb.length; i++) {
				tableBody += "<tr><td><table>"
						  ...
						  + "<a onmouseover=\"makeDivVisibleRR('rrSentences"+index+i+"');\" onmouseout=\"makeDivHiddenRR('rrSentences"+index+i+"');\">"
						  + "<b><font color='#0000FF'>"
						  + "<input class='rrPart' id='rrGroup"+index+i+"' type='checkbox' value='GROUP' onclick='javascript:otherForm(\"rrChkGrp"+index+i+"\",\"rrGroup"+index+i+"\");'>"+final_comorb[i][1]
						  + "</font></b></a></div>";
				...
				for(var j=2; j<final_comorb[i].length; j++) {		
					...
					tableBody += "<br><input class='rrPart"+index+i+"' id='rrGrp"+index+i+"Chk"+j+"' type='checkbox'>"+final_comorb[i][j][k];
					}		
				}
