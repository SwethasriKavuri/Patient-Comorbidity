...
var reportSearchJSON = [
	...
	{"category":["ECG"],
	 "report": [
		...
		{"search" : "1st degree AV block",
		 "display" : "1st degree A-V block",
		 "level" : []			
		},
		{"search" : "first degree A-V block",
		 "display" : "1st degree A-V block",
		 "level" : []			
		},
		{"search" : "first degree AV block",
		 "display" : "1st degree A-V block",
		 "level" : []			
		}
	 ]}
			...
			if (msgText != undefined && msgText != null){
				...
				if (jsonObj){					
					...
					for (var x = 0; x < jsonObj.TEMP.RADCNT; x++) {
						...
						var tracelink = "http://www.google.com"
						try {
							tracelink = jsonObj.TEMP.RAD[x].EKGLINK;

						} catch (err) {
							tracelink = "http://www.google.com";
							// tracelink is not part of response yet!
						}						
...
function indexOfIgnoreCase(stringinput ,searchvalue, start) {
	var input;
	var search;
	var returnindex = -1;
...
function transform_report(data) {
    ...
    for (var i = 0; i < keywords.length; i++) {
		pos = indexOfIgnoreCase(input_data,keywords[i]);
		if (pos != -1)
		{
			keyword_index = i;
			break;
    ...
    while (pos != -1) 
    {
        ...
        var end_pos = indexOfIgnoreCase(input_data,keywords[keyword_index]);
		...
		{
			if (indexOfIgnoreCase(input_data, "Performed on Date") == -1)
			{
...
function find_next_keyword(keywords, index, input_data, pos) {
    ...
    for (var i = index + 1; i < keywords.length; i++) {
        ...
        var next_keyword_pos = indexOfIgnoreCase(input_data,keyword, pos);
...
function linkReportInPreviouslyCommitted(pcComorb, reportCritArr, reportIndex, source)
{
	...
	if (source === 'ECG') {
		...
		tracelink = ecgTraceLinks[reportIndex];
	}
	else
		link = radiologyLinks[reportIndex];
	var html = "<br/><a href='"+link+"'> Link to Report </a>&nbsp;&nbsp;<button onclick=\"showPDF("+reportIndex+", true,'"+source+"');makeDivHidden('"+domId+"');\">Show Report</button>&nbsp;&nbsp;<a href='"+tracelink+"'>Link to ECG Tracing </a>"
...
function showPDF(index, isCommitted, source) {
		...
		for (var i = 0; i < array3.length; i++) {
	        ...
	        for (l = 0; l < conjunctions_two_d.length; l++) {
	            ...
	            for (m = 0; m < conjunctions_two_d[l].length; m++) {
	                ...
	                if (!isAWordMatch(cur_string, conjunctions_two_d[l][m])) {
	                    conj_present = 0;
	                    break;
	                }
	            }
	        ...
	        if(pos_conj == 1) {
	            pos_positive_connotation = cur_string.toLowerCase().search(positive_connotation_string.toLowerCase());
	            var pos_connotation_part = cur_string.slice(pos_positive_connotation, cur_string.length);
	            var pos_comorb = pos_connotation_part.toLowerCase().search(array3[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
	            if (pos_comorb != -1)
	                pos = pos_positive_connotation + pos_comorb;
	            else if (pos_comorb == -1)
	                pos = cur_string.toLowerCase().search(array3[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
	        } else
	            pos = cur_string.toLowerCase().search(array3[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
	...
	tableBody += "<br/><a href='"+link+"'> Link to Report </a>"
	if (source == 'ECG')
		tableBody += "&emsp;&emsp;<a href='"+tracelink+"'>Link to ECG Tracing </a>"
			...
			for (var i = 0; i < final_comorb.length; i++) {
				...
				for (var j = 0; j < impression_sentences.length; j++) {
					...
					for (l = 0; l < conjunctions_two_d.length; l++) {
						...
						for (m = 0; m < conjunctions_two_d[l].length; m++) {
							positive_connotation_string += conjunctions_two_d[l][m];
							if (!isAWordMatch(impressionsentence, conjunctions_two_d[l][m])) {
								conj_present = 0;
								break;
							}
						}
					...
					if (pos_conj == 1) {
						pos_positive_connotation = impressionsentence.toLowerCase().search(positive_connotation_string.toLowerCase());
						var pos_connotation_part = impressionsentence.slice(pos_positive_connotation, impressionsentence.length);
						var pos_comorb = pos_connotation_part.toLowerCase().search(final_comorb[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
						if (pos_comorb != -1)
							pos = pos_positive_connotation + pos_comorb;
						else if (pos_comorb == -1)
							pos = impressionsentence.toLowerCase().search(final_comorb[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
					} else
						pos = impressionsentence.toLowerCase().search(final_comorb[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
				...
				tableBody += "<br/><a href='"+radiologyLinks[index]+"'> Link to Report </a>";			
			...
			for (var i = 0; i < final_comorb1.length; i++) {
				...
				for (j = 0; j < findings_sentences.length; j++) 
				{
					...
					for (l = 0; l < conjunctions_two_d.length; l++){
						...
						for (m = 0; m < conjunctions_two_d[l].length; m++){
							positive_connotation_string += conjunctions_two_d[l][m];
							if (!isAWordMatch(findingssentence, conjunctions_two_d[l][m])) {
								conj_present = 0;
								break;
							}
						}
					...
					if (pos_conj == 1){
						pos_positive_connotation = findingssentence.toLowerCase().search(positive_connotation_string.toLowerCase());
						var pos_connotation_part = findingssentence.slice(pos_positive_connotation, findingssentence.length);
						var pos_comorb = pos_connotation_part.toLowerCase().search(final_comorb1[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
						if (pos_comorb != -1)
							pos = pos_positive_connotation + pos_comorb;
						else if (pos_comorb == -1)
							pos = findingssentence.toLowerCase().search(final_comorb1[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
					} else
						pos = findingssentence.toLowerCase().search(final_comorb1[i].search.toLowerCase().replace(/^\s+|\s+$/gm,""));
...
function determine_final_comorb(impression_sentences,comorb_array){
	...
	for (i = 0; i < comorb_array.length; i++){
		var temp_array = new Array();
		temp_array.push(comorb_array[i].search);
